...
@patch('documents.tasks.send_document_confirmation_notification.delay') # Подменить функцию
@patch('documents.tasks.send_document_rejection_notification.delay') # Подменить функцию
def test_save_model(mock_reject_task, mock_confirm_task, document_admin, admin_user):
    ...
    document = Document.objects.create(title='Test Document', file='test.pdf', user=admin_user)
    ...
    mock_confirm_task.assert_called_once_with(document.user.email, document.admin_comment)
    ...
    assert document.processed_at is not None
    # Проверить отправку уведомления подтверждения документа
    mock_reject_task.assert_called_once_with(document.user.email, document.admin_comment)
